...
function cancelButon(setShowModal){
  setShowModal(false)
}


export default function Modal({ setShowModal,setSave, resultt, setColumns, columns}){
  const [comment, setComment] = useState('')

  const { source, destination } = resultt;
    ...
    <ModalBackground>
      <ModalContainer>
        ...
        <div className="btn-group">
          <Button type="normal link" text="Iptal Et" onClick={() => cancelButon(setShowModal)}  />
          <Button type="normal" text="Kaydet" onClick={() => saveComment(setSave)} />
        </div>
      </ModalContainer>
    </ModalBackground>
  );
